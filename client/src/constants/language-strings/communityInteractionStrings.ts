interface CommunityInteractionStrings {
  pageTitle: {
    english: string;
    spanish: string;
  };

  givingBackToCommunity: {
    part1: {
      english: string;
      spanish: string;
    };
    part2: {
      english: string;
      spanish: string;
    };
  };

  salvationArmy: {
    title: {
      english: string;
      spanish: string;
    };
    body: {
      english: string;
      spanish: string;
    };
    image: string;
  };

  charlotteRescueMission: {
    title: {
      english: string;
      spanish: string;
    };
    body: {
      english: string;
      spanish: string;
    };
    image: string;
  };

  thompsonFamilyFocus: {
    title: {
      english: string;
      spanish: string;
    };
    body: {
      english: string;
      spanish: string;
    };
    image: string;
  };
}

export const communityInteractionStrings: CommunityInteractionStrings = {
  pageTitle: {
    english: "Community Involvement",
    spanish: "Participación de la Comunidad",
  },

  givingBackToCommunity: {
    part1: {
      english:
        "At Olive Sanchez & Associates, we pride ourselves in giving back to our community.",
      spanish:
        "En Olive Sanchez & Associates, nos enorgullecemos de contribuir a nuestra comunidad.",
    },
    part2: {
      english: "We proudly support the following:",
      spanish: "Estamos orgullosos de apoyar lo siguiente:",
    },
  },

  salvationArmy: {
    title: {
      english: "Salvation Army",
      spanish: "Ejército de Salvación",
    },
    body: {
      english:
        "In past years, Olive Sanchez & Associates has participated in the Salvation Army’s Angel Giving Tree Project to help give needy children the Christmas they deserve.",
      spanish:
        "En los últimos años, Olive Sanchez & Associates ha participado en el Proyecto Angel Giving Tree del Ejército de Salvación para ayudar a darles a los niños necesitados la Navidad que se merecen.",
    },
    image: "",
  },

  charlotteRescueMission: {
    title: {
      english: "Charlotte Rescue Mission",
      spanish: "Misión de Rescate de Charlotte",
    },
    body: {
      english:
        "Olive Sanchez & Associates donates to the Charlotte Rescue Mission, which provides meals to individuals caught in the cycles of poverty, homelessness and chemical addictions.",
      spanish:
        "Olive Sanchez & Associates dona a Charlotte Rescue Mission, que proporciona comidas a personas atrapadas en los ciclos de pobreza, falta de vivienda y adicciones químicas.",
    },
    image: "",
  },

  thompsonFamilyFocus: {
    title: {
      english: "Thompson Family Focus",
      spanish: "Enfoque Familiar Thompson",
    },
    body: {
      english:
        "Members of Olive Sanchez & Associates have volunteered at Thompson Child and Family Focus, which provides comprehensive education, treatment and care for children in need.",
      spanish:
        "Los miembros de Olive Sanchez & Associates han sido voluntarios en Thompson Child and Family Focus, que brinda educación, tratamiento y atención integrales a niños necesitados.",
    },
    image: "",
  },
};
